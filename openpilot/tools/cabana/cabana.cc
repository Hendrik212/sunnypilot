...
void printUsage(const char *argv0) {
  fprintf(stderr,
          ...
          "  --mcp-server              start MCP server on default port (3001)\n"
          "  --mcp-port <port>         specify MCP server port\n",
          argv0);
}
...
int parseArgs(int argc, char *argv[], CabanaArgs &args, bool &ok) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (std::strcmp(a, "--mcp-server") == 0) {
      args.mcp_server = true;
    } else if (std::strcmp(a, "--mcp-port") == 0) {
      if (!takeValue(argc, argv, i, args.mcp_port)) return 1;
    } else if (a[0] == '-') {
...
int main(int argc, char *argv[]) {
  ...
  if (args.mcp_server) {
    uint16_t mcpPort = 3001;
    if (!args.mcp_port.empty()) {
      try {
        size_t pos = 0;
        int port = std::stoi(args.mcp_port, &pos);
        if (pos != args.mcp_port.size() || port <= 0 || port > 65535) throw std::invalid_argument("range");
        mcpPort = static_cast<uint16_t>(port);
      } catch (const std::exception &) {
        fprintf(stderr, "Invalid MCP server port %s, using default 3001\n", args.mcp_port.c_str());
        mcpPort = 3001;
      }
    }
